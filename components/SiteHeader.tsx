    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-cocoa/5 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-3 md:hidden">
          {/* Mobile Profile Avatar Visual Anchor */}
          {isLoggedIn && (
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-berry/20 to-cocoa/10 border border-berry/30 flex items-center justify-center font-bold text-xs text-berry shadow-inner select-none">
              {userInitial.toUpperCase()}
            </div>
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out border-cocoa/5 bg-white
          ${isMenuOpen ? "max-h-[500px] border-t opacity-100 visible" : "max-h-0 opacity-0 invisible"}`}
      >
          ...
          {isLoggedIn ? (
            <>
              ...
              {isBakeryOwner && (
                <Link
                  href="/dashboard"
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full text-center font-semibold text-white bg-cocoa hover:bg-cocoa/90 px-4 py-3 rounded-xl shadow-sm transition-all block mt-2"
                >
                  ⚙️ Bakery dashboard
                </Link>
              )}

              {/* Mobile Log Out CTA Button */}
              <button
                onClick={logout}
                disabled={isLoggingOut}
                className="w-full text-left px-4 py-3 rounded-xl font-medium text-red-500 hover:bg-red-50 active:bg-red-100 transition-all duration-200 disabled:opacity-50 flex items-center gap-2 mt-4 border-t border-cocoa/5 pt-4"
              >
                {isLoggingOut ? (
                  <>
            ...
            <div className="w-full space-y-3 pt-2">
              <Link
                href="/login"
                onClick={() => setIsMenuOpen(false)}
                className="w-full text-center block font-semibold text-cocoa/80 hover:bg-cocoa/5 py-3 rounded-xl transition-all"
              >
                Log in
              </Link>
              <Link
                href="/register"
                onClick={() => setIsMenuOpen(false)}
                className="w-full text-center block font-semibold bg-berry hover:bg-berry-dark text-white py-3 rounded-xl shadow-sm transition-all"
              >
                Sign up
              </Link>
            </div>
